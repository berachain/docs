...
- [**PoL vNext ABIs**](/general/proof-of-liquidity/files/pol-vnext-abis.zip) — 89 JSON ABI files; 222 KB (updated 2026-05-14)
- [**PoL vNext contract source**](/general/proof-of-liquidity/files/pol-vnext-contracts.zip) — 3.5 MB (updated 2026-05-14)

### Change summary

#### [PoL contracts](https://github.com/berachain/contracts) changes:

- **`IBlockRewardController`**
  - **Removed:** `boostMultiplier()`, `computeReward()`, `minBoostedRewardRate()`, `rewardConvexity()`, `setBaseRate(uint256)`, `setBoostMultiplier()`, `setMinBoostedRewardRate()`, `setRewardConvexity()`, `setRewardRate(uint256)`.
  - **Removed events:** `MinBoostedRewardRateChanged`, `BoostMultiplierChanged`, `RewardConvexityChanged`.
  - **Added:** `burnExceedingBalance()`, `getMaxEmissionPerBlock()`.
  - **Added events:** `ExceedingBalanceBurnt`.
- **`IDistributor`**
  - **Added:** `EmissionTokenSet` event, `emissionToken()` getter, `setEmissionToken()` setter.
- **`IRewardVault`**
  - **Updated:** `whitelistIncentiveToken`, `exit`, `getReward`, `getPartialReward`, `addIncentive` (parameters and documentation updated to reflect the shift from BGT to the new emission token).
  - **Renamed events:** `BGTBoosterIncentivesProcessed` → `IncentivesCollected`, `BGTBoosterIncentivesProcessFailed` → `IncentivesCollectionFailed`.
  - **Updated events:** `IncentivesProcessed`, `IncentivesProcessFailed`, `IncentivesCollected`, and `IncentivesCollectionFailed` rename the `bgtEmitted` argument to `rewardsEmitted`.
  - **Removed events:** `IncentiveFeeCollected`, `IncentiveFeeCollectionFailed`.
  - **Added events:** `RewardTokenMigrated`.
- **`IRewardVaultFactory`**
  - **Removed:** `BGTIncentiveDistributorSet` event, `IncentiveFeeRateUpdated` event, `IncentiveFeeCollectorUpdated` event, `setBGTIncentiveDistributor()`, `setBGTIncentiveFeeRate()`, `setBGTIncentiveFeeCollector()`, `bgtIncentiveDistributor()`, `bgtIncentiveFeeRate()`, `bgtIncentiveFeeCollector()`, `getIncentiveFeeAmount()`.
  - **Added:** `IncentiveTokensCollectorUpdated` event, `setIncentiveTokensCollector()`, `incentiveTokensCollector()`.
- **`IRewardVaultHelper`**
  - **Added:** `SWBERASet` event, `RewardsClaimed` event, `claimAllRewards(address[], address, address)` (new overload accepting an output token).
- **`IWBERA`**
  - **Added:** New interface for WBERA (`deposit()`, `withdraw()`).

#### [Staking pools](https://github.com/berachain/contracts-staking-pools) changes:

- **`ISmartOperator`**
  - **Added:** `WBERADepositedToPool` event, `WBERASentToWithdrawalVault` event.
  - **Added:** `accrueEarnedWBERAFees()`, `withdrawRewards(uint256)`, `pullBeraToWithdrawalVault(uint256)`, `rebaseableWberaAmount()`, `availableWBERABalance()`, `getEarnedWBERAFeeState()`.
  - **Deprecated (retained for transition only):** `queueBoost()`, `activateBoost()`, `queueDropBoost(uint128)`, `claimBgtStakerReward()`, `claimBoostRewards(IBGTIncentiveDistributor.Claim[],address[])`. BGT-oriented entrypoints stay callable while residual BGT settles, then become inert.
- **`IStakingPool`**
  - No signature changes; `receiveRewards()` natspec updated (caller restriction lifted from `StakingRewardsVault`).
- **`IWithdrawalVault`**
  - **Added events:** `FullExitRequestRetried`.
