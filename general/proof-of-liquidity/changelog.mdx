...
This upgrade deployed to the network approximately 24 hours before the [Fusaka upgrade](/build/protocol/overview#fusaka-upgrade-features).
...
**Timeline**: These Proof of Liquidity changes deployed 24 hours before the [Fusaka hardfork](/build/protocol/overview#fusaka-upgrade-features).

| User / Actor         | Expected Action                                                                                                                                                            |
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| BGT holders          | Redeem BGT directly or migrate through the [Hub](https://hub.berachain.com) UI, following Fusaka activation ([timeline](/build/protocol/overview#fusaka-upgrade-features)) |
...
**[Reward vault rate-based emissions](/general/proof-of-liquidity/reward-vaults#wbera-emission-timing-modes)** — Reward vault managers can configure emissions to flow at a target per-second rate; the distribution period is computed automatically from the reward amount and target rate.
