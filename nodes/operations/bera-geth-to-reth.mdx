...
2. **Extract** — Extract into the location of Reth's `--datadir`. For the suggested tree, datadir is `/srv/bera/reth/data`, so unpack like so:

```bash
lz4 -d mainnet-reth-archive*lz4 | tar xv -C /srv/bera/reth/data/
```

If Reth partially synced earlier, wipe `reth/data` before extracting.

## Start

Start `beacond` and `bera-reth` with whatever supervisor you already use. We recommend EL, then CL.
Provided JWT path and the Reth authrpc port agrees with the Beacond `rpc-dial-url`, they should connect and begin following the chain.
