...
`reth` allows you to enable metrics with the following command line options:
...
The address should be either on a private network or not accessible to the public via firewall rule. `reth` publishes the metrics at `/metrics`.
