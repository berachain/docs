...
The `setup-reth.sh` script creates a runtime directory and configuration for the execution client. The script configures the node with pruning settings according to the `EL_ARCHIVE_NODE` setting in `env.sh`.
