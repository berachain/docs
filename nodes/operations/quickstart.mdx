...
- Or, use the `fetch-berachain-snapshot.js` script to download official Berachain snapshots. The script is **standalone**: it does not read `env.sh` or any other local config. It uses Node.js 18+ built-ins only (no `curl`). It fetches the index from `snapshots.berachain.com` and downloads the latest beacon-kit and execution snapshots that match your flags.

```bash
# FROM: ~/beranode (or any directory — pass -o for output path)

# Default: reth, mainnet, pruned, writes to ./downloads
node fetch-berachain-snapshot.js

# Or specify options (no env file required):
...
- `--index-url`: Override the snapshot index CSV URL (default: `https://snapshots.berachain.com/index.csv`)
- `--el-client`: Execution snapshot type prefix in the index (default: `reth`)
