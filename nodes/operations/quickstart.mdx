...
<Info>
  Do this step **before** starting your clients (Step 8). If you've already started syncing, you'll
...
- Or, use the `fetch-berachain-snapshot.js` script to download official Berachain snapshots. The script does not read `env.sh`. It loads the snapshot **index CSV** with Node.js 18+ `fetch`, then runs **`curl`** (must be on your `PATH`) for each file so downloads can resume (`curl -C -`). By default, mainnet uses `https://snapshots.berachain.com/index.csv` and testnet (`-n testnet`) uses `https://bepolia.snapshots.berachain.com/index.csv`. The script picks the newest matching beacon-kit and reth rows and prefers `url_s3` from the CSV when present.

```bash
# FROM: ~/beranode (or any directory — use -o to change download folder)

# Default: mainnet, pruned, writes to ./downloads
...
If `tar -tv` on your file shows paths starting with `data/` instead, unpack with `-C` set to the **parent** of `var/reth/data` (i.e. `var/reth/`) so the archive's `data/` directory becomes your datadir.
