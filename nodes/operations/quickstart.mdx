...
# fetch-berachain-snapshot-v2.js
```
...
Official Berachain snapshots are storage v2 (Bepolia live; mainnet still uses the v1 script). Storage v1 datadirs and community v1 packs remain valid. Full restore commands, Reth layout rules, and the catalog pairing live on [Snapshots](/nodes/operations/snapshots).

```bash
# FROM: ~/beranode

# Official v2 (Bepolia). Creates downloads/bepolia-pruned-{el,cl}
node fetch-berachain-snapshot-v2.js --network bepolia --type pruned

# Official v1 (Mainnet today), or any v1 index.csv pack
node fetch-berachain-snapshot.js --network mainnet --type pruned
```

Community sources: [Awesome Berachain Validators](https://github.com/chuck-bear/awesome-berachain-validators).
