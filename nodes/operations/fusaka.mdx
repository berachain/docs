...
Two quality-of-life changes land with this release of Bera-Reth, simplifying future upgrades.

1. `--chain (bepolia|mainnet)` instead of a full path to the genesis file. You no longer have to deploy new genesis files. The usual style `--chain <path>` still works, but you are responsible for fetching and installing updated genesis files.
2. If you use `--chain (bepolia|mainnet)`, then `--bootnodes BN1,BN2,...` can be removed. Bera-Reth knows which bootnodes to use based on the chain setting.
...
4. Adjust Bera-Reth startup:
   - Change your `--chain` option to `--chain bepolia`
   ...
   - You may also remove the `--bootnodes` option to Bera-Reth.
