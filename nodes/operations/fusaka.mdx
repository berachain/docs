...
<Info>
  Bepolia has a follow-on execution-layer hardfork, **Osaka1**, activating at 4pm UTC on July 22, 2026. See the [Osaka1 upgrade guide](/nodes/operations/osaka1) for the current Bepolia versions and genesis file.
</Info>
...
3. Adjust Bera-Reth startup:
   - Change your <span style={{ whiteSpace: "nowrap" }}>`--chain`</span> option to <span style={{ whiteSpace: "nowrap" }}>`--chain bepolia`</span> or <span style={{ whiteSpace: "nowrap" }}>`--chain mainnet`</span>, and remove <span style={{ whiteSpace: "nowrap" }}>`--bootnodes [enode-list]`</span>.
