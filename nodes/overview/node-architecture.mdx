...
flowchart TB
  subgraph N1["🐻⛓️ Node"]
    direction TB
    R1["Bera Reth<br/><small>(Execution Client)</small>"]:::evm
    B1["Beacond<br/><small>(Full Node)</small>"]:::cons
    R1 <== "Engine API" ==> B1
  end
  subgraph N2["Node"]
    direction TB
    R2["Bera Reth<br/><small>(Execution Client)</small>"]:::evm
    B2["Beacond<br/><small>(Light Node)</small>"]:::cons
    R2 <== "Engine API" ==> B2
  end
  subgraph N3["Node"]
    direction TB
  ...
  classDef evm fill:#f7b733,stroke:#ba7517,stroke-width:2px,color:#412402
  classDef cons fill:#f26b1d,stroke:#a8410f,stroke-width:2px,color:#ffffff
```

<Frame>
  ![Berachain node architecture showing execution and consensus client pairs](/images/nodes/berachain-node-architecture.png)
</Frame>
...
An RPC node can become a validator node by joining the [Active Set](#validator-active-set) through interaction with the [BeaconDeposit](/build/getting-started/deployed-contracts) contract by meeting the \$BERA [stake requirements](#validator-active-set). To learn about the economics of becoming a validator, see the [Validator Lifecycle](/nodes/architecture/validator-lifecycle) and the [Be A Validator](/nodes/guides/become-a-validator) guides.
