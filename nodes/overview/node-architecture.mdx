...
description: "Validator and RPC node architecture: consensus and execution separation, Engine API, dual P2P networks, and Proof of Gossip."
---
...
To manage the complexity of the execution P2P network, infrastructure operators can run **Proof of Gossip (PoG)**.

PoG consists of two components:

- **Bera-Sentinel:** An operational fleet manager that orchestrates peer quality across your nodes.
- **Bera-Reth (Execution Client):** Emits canary transactions to test whether specific devp2p peers are actively relaying transactions to the rest of the network.

By using PoG, operators can actively drop "zombie" peers that aren't actually contributing to transaction inclusion. To learn more about deploying this sidecar, see the [Proof of Gossip](/nodes/proof-of-gossip/index) section.
