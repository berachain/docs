...
- **Active**  
  The validator is marked as Active after 1 epoch from the Eligible state. Currently, the Active Set is capped to a maximum of **69** Validators.
...
The validator's journey begins with a deposit transaction on the **Execution Layer** to the deposit contract at `0x4242424242424242424242424242424242424242`. The deposit emits a custom deposit event; the consensus client parses that event from the execution payload's request list and processes the deposit in the **same block** the deposit transaction was included in.
...
- **In-payload, per-block processing**
  Deposits are included in execution payloads and processed by the CL in the same block. There is no follow distance, no pre-fork queue, and no per-epoch cadence. Once the EL block carrying the deposit transaction is finalized, the deposit is reflected in the validator state.
