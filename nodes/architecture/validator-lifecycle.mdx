...
flowchart TB
  ...
  subgraph CL["Consensus Layer"]
    ...
    ACT["Active"]
    EXT["Exited"]
    DEP -- "1 epoch delay" --> ELG
    ELG -- "1 epoch delay" --> ACT
    ELG -- "1 epoch delay<br/>if validatorSetCap full<br/>and low priority val" --> EXT
    ACT -- "if validatorSetCap full<br/>& low priority val" --> EXT
  end
  subgraph EL2["Execution Layer"]
    WD["Withdrawn<br/><small>Funds withdrawn to withdraw credential</small>"]
  end
  DC -. "2 ETH1 block delay" .-> DEP
  EXT -. "1 epoch delay<br/>All funds fully withdrawn" .-> WD
```

<Frame>
  ![Extended validator lifecycle diagram showing all state transitions including forced exits](/images/nodes/validator-extended-lifecycle.png)
</Frame>
...
Once active, validators receive block rewards for proposing blocks. Block rewards are in the form of `$BGT`, with a base reward of **0.4** `$BGT` per block proposed.
