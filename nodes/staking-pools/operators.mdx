...
| Minimum Effective Balance | ≥ `MIN_EFFECTIVE_BALANCE` (250,000 BERA)   | Activation threshold and full exit safeguard      |
| Minimum CL Deposit        | `MIN_CL_DEPOSIT_AMOUNT` (10,000 BERA)      | Smallest consensus-layer deposit the pool batches |
...
Operator action is not required to make WBERA reach stakers. Compounding fires automatically inside any user `submit()` (or a plain BERA transfer to the pool) and inside `processRewards()`; covering withdrawals from operator-side WBERA fires automatically inside `WithdrawalVault.requestWithdrawal()`. See [Automatic WBERA flows](#automatic-wbera-flows) below.
...
A pool's stBERA share price tracks `totalAssets()` on the `StakingPool`. Consensus-layer validator balance is the largest component of that NAV, but the execution layer does not read beacon state directly on every block.

The shared **AccountingOracle** is the only path from a validator's consensus-layer balance to on-chain `totalDeposits`. An address with `FEEDER_ROLE` on the oracle calls `updateTotalDeposits` with a beacon block root, a pubkey proof, and a balance proof. The `StakingPoolContractsFactory` verifies those proofs before the oracle forwards the balance to the pool's `updateTotalDeposits(uint256 newTotalDeposits)`.

The write is not a trusted push. The factory checks the pubkey and balance proofs against the reported beacon block root before any pool state changes.

Between reports, `totalAssets()` reflects the **last verified** consensus-layer balance plus buffered execution-layer funds and pending operator WBERA. It does not track live beacon state in real time. Stakers should expect share price to move when a feeder publishes a new verified report, not on every consensus-layer slot.
...
The `minEffectiveBalance` parameter is critical for validator activation. On-chain, `StakingPool.MIN_EFFECTIVE_BALANCE` is **250,000 BERA** and is the floor for any custom `minEffectiveBalance` you set. When the validator set is full (69 validators), the consensus layer minimum increases in increments of **`MIN_CL_DEPOSIT_AMOUNT` (10,000 BERA)** above the lowest active validator. Set `minEffectiveBalance` to match the current consensus layer requirement. You can check the current lowest active stake on [Berachain Hub](https://hub.berachain.com/boost/).
...
## Deposits and the DepositSubmitted event

Stakers fund the pool through `submit(address receiver)` or by sending native BERA to the pool. Each successful deposit emits `DepositSubmitted` with:

- `receiver` — the address that receives minted stBERA shares
- `userDepositAmount` — the BERA amount credited from this deposit
- `shares` — stBERA shares minted to the receiver
- `rewardsCollected` — staking-rewards vault balance swept during the deposit
- `bufferedAssets` — execution-layer BERA waiting to batch to the consensus layer after the deposit
- `totalDeposits` — the pool's consensus-layer deposit total after the deposit logic runs

Front-ends can index this event to show deposit history without replaying internal accounting.
...
**`coveredByOperator` accounting:** `WithdrawalVault.requestWithdrawal()` forwards a `coveredByOperator` amount into `StakingPool.notifyWithdrawalRequest(...)`. That value is the portion of the withdrawal satisfied from operator WBERA at request time. The pool decrements `totalDeposits` only by `assets - coveredByOperator`, because only the consensus-layer portion leaves the validator balance. Operator-covered BERA never counted as consensus-layer stake, so this keeps pool accounting aligned with actual CL outflow. The parameter is ignored on short-circuit and full-exit branches.
