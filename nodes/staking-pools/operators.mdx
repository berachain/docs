...
description: "Roles, commission, reward allocation, min effective balance, withdrawals, and front-end."
---

Day-two work after `install.sh`: roles, commission, reward allocation, min effective balance, WBERA flows, and withdrawals. Deploy first with [Install a staking pool](/nodes/staking-pools/installation).
...
## What you need before you operate a pool
...
Unboost, drop-boost, and redeem BGT held on your `SmartOperator` with those same calls during the transition window.
...
Berachain provides a React-based example template in the [guides repository](https://github.com/berachain/guides/tree/main/apps/staking-pools/frontend). Copy `config.example.json`, set your pool address and validator pubkey (from `install.sh` output or factory `getCoreContracts`), and deploy.

## Delegation

If you received a Foundation delegation, see [Run a pool with Foundation-delegated capital](/nodes/staking-pools/delegators).

## What's next

Continue with:

- [What staking pools do](/nodes/staking-pools/overview)
