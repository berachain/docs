...
Berachain provides a React-based example template in the [guides repository](https://github.com/berachain/guides/tree/main/apps/staking-pools/frontend). On the validator, run `generate-frontend-config.sh` from `install-helpers` (with `BEACOND_HOME` in `env.sh`) to write `config.draft.json`.
...
- [Installer README](https://github.com/berachain/guides/blob/main/apps/staking-pools/installer/README.md) — remote landing
