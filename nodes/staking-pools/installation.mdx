...
- `jq`, `bc`, `curl`
- Ledger hardware wallet (default) or a private key set via PRIVATE_KEY in env.sh
...
2. Confirms the operator address registered for your validator pubkey matches the pool's SmartOperator and that the validator's withdrawal credentials point at the pool's WithdrawalVault.
