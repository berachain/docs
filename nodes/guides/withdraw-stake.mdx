...
<Warning>
  ...
  produce a block and therefore reduce your Reward Vault emissions. Withdrawing all stake will exit
  your validator from the Active Set.
</Warning>
...
<Tip>The beacon API must be enabled on your node (in `app.toml`: `[beacon-kit.node-api]`).</Tip>
