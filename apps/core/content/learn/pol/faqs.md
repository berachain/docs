...
## PoL V2 and BERA Staking 🐻

### What is PoL V2?

PoL V2 brings the BERA Yield Module, which gives `$BERA` holders direct yield opportunities through a new staking vault called WBERAStakerVault. This lets BERA holders earn yield from redirected PoL incentives without diving into complex DeFi protocols.
...
During the 7-day period, you can't earn additional rewards, and you can't cancel the withdrawal request.
...
No, all existing Reward Vault functionality remains unchanged. The incentive tax only affects the distribution of incentives, not the core PoL mechanics. Protocols continue to compete for BGT emissions as before.
...
Currently, staked BERA in the WBERAStakerVault cannot be delegated to validators. However, future versions will support Liquid Staking Tokens (LSTs), which will enable dual yield from both validator staking and PoL incentives.
