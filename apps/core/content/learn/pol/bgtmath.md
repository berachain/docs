...
BGT APR % is calculated using ranges of blocks, defined by a starting block and an ending block. At the time the percentages are calculated, the APR calculator samples the prices of all tokens (in $BERA).
...
$$ R_c = \frac{\sum_{T∈IT}\Big(\sum_{b = B_s}^{B_e} I_{T,b}\Big) \times P_{T,t_{B_e}}} {Boost_{t_{B_e}} \times P_{BERA,t_{B_e}}} $$

$$ APR = R_c \times \frac{{T_{year}}}{t_{B_e} - t_{B_s}}  $$
...
$$ I_{T,b} = IR_T \times DistributedBGT \times (1 - C) $$
