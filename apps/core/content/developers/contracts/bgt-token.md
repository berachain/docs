...
*Should be owned by the governance module.*

*Only allows minting BGT by the BlockRewardController contract.*

*It's not upgradable even though it inherits from `ERC20VotesUpgradeable` and `OwnableUpgradeable`.*

*This contract inherits from `Multicallable` to allow for batch calls for `activateBoost` by a third party.*
