...
The deployment function requires five key parameters. Your validator's public key (in 48-byte hex format) serves as the unique identifier for the pool. The withdrawal credentials must be calculated based on the predicted withdrawal vault address. The signature is generated by your beacon client for the initial deposit. The validator admin address will have administrative privileges over the pool, typically a governance multisig for security. The default share recipient address will receive the initial shares in the pool, usually your validator wallet.
...
```solidity
// Queue validator commission rate change (in basis points, max 1000 = 10%)
