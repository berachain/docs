...
Having completed the registration, you can now deposit additional $BERA. The floor for becoming a validator is `{{ config.mainnet.minActivationBalance.toLocaleString() }} $BERA`, and you must be among the top `{{ config.mainnet.validatorActiveSetSize }}` validators, ordered by $BERA staked.
