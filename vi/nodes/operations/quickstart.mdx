...
The `setup-reth` script creates a runtime directory and configuration for the execution client. The script configures the node with pruning settings according to the `EL_ARCHIVE_NODE` setting in `env.sh`.
...
- `--client` or `-c`: Execution client (default: `reth`)
...
EL_SNAPSHOT=$(ls downloads/snapshot_reth-*.tar.lz4 2>/dev/null | head -1)
