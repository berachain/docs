---
title: "Single Swap"
description: "Exchange one token for another within a single BEX liquidity pool using the swap function on the Vault contract."
---

A single swap in BEX allows you to exchange one token for another within a single liquidity pool by calling the `swap` function on the BEX Vault. This function is implemented in the [Vault contract](/build/bex/contracts/vault), which serves as the main entry point for all BEX operations.
