...
<Frame>
  ![BEX pool creation interface showing token selection and initial ratio configuration](/images/bex/bex-create-pool-stable-tokens.png)
</Frame>

## Selecting the ratio

During pool creation, the price/ratio of the tokens is set based on amount of "quote" tokens per "base" token. Using the above scenario as an example, and assuming that the price of `$WETH` is $$3000, the initial price of `$$BUSD`is set from the initial`3000\` ratio, that is:

- Each `$WETH` is worth `3000` `$BUSD`
- Each `$BUSD` is worth `0.000333` `$WETH`
- The initial price of `$BUSD` is `$3000 * 0.000333 = $1`
