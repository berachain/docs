...
description: "Multi-token pools with configurable weights for uncorrelated assets; exposure and impermanent loss."
---

Weighted pools are an extension of traditional AMM pools (i.e., Uniswap V2's 50/50 `x = y * k` pools). Weighted pools are suitable for very generalized use cases, particularly for pairs of tokens without price correlation (e.g., `BUSD/WETH`). BEX weighted pools allow you to create pools with multiple tokens and custom weight distributions, such as `80/20` or `60/20/20` ratios.
