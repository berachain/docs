...
Validators can operate independently with direct staking, or they can operate **staking pools** that allow community members to stake BERA through smart contracts and receive liquid shares (stBERA). Staking pools follow the same validator lifecycle described in this document. For information about operating staking pools, see the [Staking Pools documentation](/validators/staking-pools/overview).
...
<Frame>
  <img src="/images/validators/validator-lifecycle.png" alt="Diagram showing the Berachain validator lifecycle states: Deposited, Eligible, Active, Exited, and Withdrawn" />
</Frame>
...
Once the validator enters the **Deposited** state at epoch `N`, the system marks it as `EligibleForActivationQueue` as soon as epoch `N+1` starts. This is guaranteed because there is no cap on the activation queue size.
...
A Validator may choose to exit by [withdrawing their complete stake](/validators/guides/withdraw-stake). Otherwise, the **only** reason for a validator to be evicted from the set (and have its funds returned) is if the ValidatorSetCap is reached and another validator with a higher priority enters. Higher priority is determined by:
...
<Frame>
  <img src="/images/validators/validator-extended-lifecycle.png" alt="Extended validator lifecycle diagram showing all state transitions including forced exits" />
</Frame>
