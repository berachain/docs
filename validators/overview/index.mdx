...
Each of these types of nodes is a pair of both an [execution client](/general/help/glossary#execution-client) and a [consensus client](/general/help/glossary#consensus-client). Berachain is a Layer 1 EVM Identical chain, which means that for the execution layer, it supports any EVM execution client paired with a consensus client and framework built by Berachain called [BeaconKit](/validators/architecture/beaconkit-consensus).
...
An RPC node can become a validator node by joining the [Active Set](#active-set) through interaction with the [BeaconDeposit](/build/contracts/beacondeposit) contract by meeting the `$BERA` [stake requirements](#validator-stake-requirements).
...
Validators can choose to leave the active set voluntarily — [learn how](/validators/guides/withdraw-stake) — or be ejected from the set if a validator with more stake joins.
...
A Validator can [withdraw all or part of their stake](/validators/guides/withdraw-stake).
...
The network does not distribute rewards automatically. You must distribute block rewards through the [Distributor](/build/contracts/distributor) contract. Distribution must occur before **8,191** seconds have passed, or validators risk losing those rewards.
