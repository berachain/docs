...
description: "Install and activate a staking pool with helper scripts: register, activate, set min effective balance, and optional stake."
---
...
<Warning>
All scripts write ready-to-run `cast` commands to files for you to review and execute. They do not send transactions without your confirmation. By default, scripts use Ledger for signing; set `PRIVATE_KEY` in `env.sh` if you prefer a local key.
</Warning>
...
<Warning>
The `minEffectiveBalance` parameter determines when your staking pool activates its validator on the consensus layer. If you don't set this value correctly, your pool may accumulate deposits without ever activating.
</Warning>
...
For details on why this matters, how to determine the correct value, and how the consensus layer minimum works, see the [Setting Minimum Effective Balance](/validators/staking-pools/operators#setting-minimum-effective-balance) section in the operator guide.
...
- [Contract reference](/validators/staking-pools/contracts) explains what you've deployed
- [Delegation guide](/validators/staking-pools/delegators) explains how to receive a Foundation delegation, which is very similar to this flow
