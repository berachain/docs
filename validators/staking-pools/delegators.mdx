...
See [Installation](/validators/staking-pools/installation) first. At minimum, set up `env.sh` in `install-helpers/`.

<Warning>
All helper scripts write `cast` commands to files; review and run them yourself. Transactions are never sent automatically. Signing defaults to Ledger; set `PRIVATE_KEY` in `env.sh` if you prefer a local key.
</Warning>
...
<Info>
If you are self-funded, use `stake.sh` from the [Installation guide](/validators/staking-pools/installation) instead of the delegated deposit flow.
</Info>

## Where to next

- [Contract reference](/validators/staking-pools/contracts) explains what you've deployed
- [Operator Guide](/validators/staking-pools/operators) explains what a staking pool operator can do
