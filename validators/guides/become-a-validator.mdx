...
- **OPERATOR_ADDRESS**: The address of the ETH account (EOA) that will become your **operator address** on the execution layer. This is used to interact with contracts such as [BeraChef](/build/contracts/berachef).
...
You will now send the above transaction onto the chain by calling the `deposit` function on the [BeaconDeposit](/build/contracts/beacondeposit) contract (`0x4242424242424242424242424242424242424242`):
