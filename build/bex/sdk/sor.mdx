---
title: "Smart Order Router (SOR) Guide"
description: "Find optimal swap routes with the BEX SDK SOR."
---
...
In this example, we use the Berancer SDK and [Ethers.js](https://docs.ethers.org/v6/) to perform a multi-hop swap from BUSD to WBTC using the optimal path.
...
// Create swap amount (e.g., 1 BUSD)
const tokenAmount = TokenAmount.fromHumanAmount(busdToken, "1");

// Fetch optimal swap paths
const { paths: sorPaths, routes } = await balancerApi.sorSwapPaths.fetchSorSwapPaths({
  chainId: CHAIN_ID,
  tokenIn: busdToken.address,
...
The SOR returns detailed routing information. For example, a BUSD to WBTC swap might route through multiple pools:
...
```javascript
{
  ...
  "hops": [
    {
      "poolId": "0x3ad1699...",
      "pool": { "symbol": "50WBERA-50BUSD-WEIGHTED" },
      "tokenIn": "BUSD",
      "tokenOut": "WBERA",
      "tokenInAmount": "1",
      "tokenOutAmount": "0.98"
