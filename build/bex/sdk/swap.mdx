---
title: "Swap Guide"
description: "Swap tokens with the BEX SDK."
---
...
In this example, we use the Berancer SDK and [Ethers.js](https://docs.ethers.org/v6/) to swap BUSD for USDC using the optimal swap path.
...
// Initialize tokens
const busdToken = new Token(CHAIN_ID, BUSD_TOKEN, 18, "BUSD");
const usdcToken = new Token(CHAIN_ID, USDC_TOKEN, 6, "USDC");

// Create swap amount (e.g., 1 BUSD)
const swapAmount = TokenAmount.fromHumanAmount(busdToken, "1");
...
const busdContract = new ethers.Contract(busdToken.address, tokenAbi, wallet);
await busdContract.approve(callData.to, swapAmount.amount);
