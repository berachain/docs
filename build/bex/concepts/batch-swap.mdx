---
title: "Batch Swap"
description: "Multi-hop swaps across BEX pools to optimize routes and pricing."
---
...
<Info>
  ...
  - **GIVEN\_IN**: Specify the exact amount of tokens to swap in. The function calculates and returns the amount of tokens you'll receive. Example: "Swap exactly 100 USDC for as much ETH as possible."
  - **GIVEN\_OUT**: Specify the exact amount of tokens to receive. The function calculates and returns the amount of tokens you need to swap in. Example: "Receive exactly 1 ETH, how much USDC do I need to swap?"
...
#### Example 1: GIVEN\_IN (\$USDC -\> \$BUSD -\> \$DAI)

In this example, you want to swap 1000 \$USDC through \$BUSD to get \$DAI. The swap is executed in two steps:

| Step | Amount | Token In | Token Out | Description |
| --- | --- | --- | --- | --- |
| 0 | 1000 | \$USDC | \$BUSD | Swap exact 1000 \$\$USDC for \$BUSD |
| 1 | 0 | \$BUSD | \$DAI | Swap all received \$BUSD for \$DAI |

By setting the second step's amount to 0, you ensure all \$BUSD received from step 0 is used in step 1.

#### Example 2: GIVEN\_OUT (\$USDC -\> \$BUSD -\> \$DAI)
...
| --- | --- | --- | --- | --- |
| 0 | 500 | \$DAI | \$BUSD | Request exact 500 \$DAI output |
| 1 | 0 | \$BUSD | \$USDC | Calculate required \$USDC input |

The swap calculates first how much \$BUSD is needed for 500 \$DAI, then how much \$USDC is needed for that amount of \$BUSD.
...
| Step | Amount | Token In | Token Out | Description |
| --- | --- | --- | --- | --- |
| 0 | 1000 | \$USDC | \$BUSD | Swap 1000 \$USDC for \$BUSD |
