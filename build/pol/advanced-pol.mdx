---
title: "Advanced PoL Integration"
description: "Proof of Liquidity Integration Guide for Non-ERC20 Protocols"
head:
  - - meta
    - property: og:title
...
<script setup>import config from '@berachain/config/constants.json';</script>

# Proof of Liquidity Integration Guide for Non-ERC20 Protocols

## Introduction

Users typically engage with Proof of Liquidity by staking ERC-20 receipt tokens into [Reward Vaults](/general/proof-of-liquidity/reward-vaults) to earn PoL rewards. However, this approach may not be suitable for all protocols.

This guide demonstrates how to integrate Berachain's Proof of Liquidity (PoL) system for protocols that do not naturally produce stakeable ERC-20 receipt tokens or that track balances internally. For example, a perpetual futures exchange may wish to reward users who open trading positions, and stop rewards when the position is closed.
...
The staked dummy token balance entitles users to earn PoL rewards as if they had staked an ERC-20 receipt token in a PoL vault themselves. This approach is enabled by the `delegateStake` and `delegateWithdraw` methods in the [RewardVault](/general/proof-of-liquidity/reward-vaults) contract.
