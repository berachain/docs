...
Users typically engage with Proof of Liquidity by staking ERC20 receipt tokens into [Reward Vaults](/general/proof-of-liquidity/reward-vaults) to earn `$BGT`. However, this approach may not be suitable for all protocols.
...
The staked dummy token balance entitles users to earn `$BGT` as if they had staked an ERC20 receipt token in a PoL vault themselves. This approach is enabled by the `delegateStake` and `delegateWithdraw` methods in the [RewardVault](/build/getting-started/deployed-contracts) contract.
