...
description: "Getting started building on Berachain: tools, networks, and contracts."
---
...
The Distributor contract is the entry point for BGT distribution. Distribution is built into the Berachain technology stack, so that block N includes transactions which perform distributions for block N-1. During that process, the Distributor:
...
<Tip>
RewardVault creation is permissionless, but governance must approve (whitelist) vaults to be included in validators' reward allocations.
</Tip>
